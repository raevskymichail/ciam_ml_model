...
class surrogate_model():
	...
	def train(self, data_size_multipl_by = 3, split_val = 0.1, test_size = 0.2):
		...
		scaler_target.fit(data_target.reshape(-1, 1))
	...
	def plot_test_true_vs_pred(self, figsize = (5, 5), title_text = "Stacked model", **kwargs):
		...
		plot_true_vs_pred(self.y_pred_test, self.y_test, title_text)

	def plot_test_true_minus_pred(self, figsize = (5, 5), title_text = "Stacked model", **kwargs):
		plt.figure(figsize = figsize)
		plot_true_minus_pred(self.y_pred_test, self.y_test, title_text)

	def plot_submodels_test_true_vs_pred(self, figsize = (10, 10), **kwargs):
		fig, axes = plt.subplots(2, 2, figsize=figsize)
		axes = axes.flatten()

		plot_true_vs_pred(self.test_pred0.values.flatten(), self.y_test, 'LightGBM model', ax=axes[0])
		plot_true_vs_pred(self.test_pred1.values.flatten(), self.y_test, 'AdaBoost model', ax=axes[1])
		plot_true_vs_pred(self.test_pred2.values.flatten(), self.y_test, 'Polynom_reg model', ax=axes[2])
		plot_true_vs_pred(self.test_pred3.values.flatten(), self.y_test, 'CatBoost model', ax=axes[3])
